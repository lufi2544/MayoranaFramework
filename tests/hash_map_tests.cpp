...
{
	SCRATCH();
	hash_map_t map = hash_map_create(temp_arena, 10, sizeof(u32), sizeof(u32), &hash_function);
...
TEST_CASE("Hash_map_init, key size is the max permitted")
{
	...
	CHECK_EQ(map.size, 10);
	CHECK_EQ(map.data_size, sizeof(u32));
	CHECK_EQ(map.key_size, g_max_key_size);
}
...
u32 hash_function_string(void* k)
{
	char* key = (char*)(k);	
	return cstr_len(key);
}
...
TEST_CASE("Hash Map, value add, <char*, u32>")
{
	...
	const char key[10] = "Hello";
	u32 data = 10;
	u32 len = cstr_len(key) + 1;
	hash_bucket_t *bucekt = hash_map_add(&map, (void*)key, len, &data);		
